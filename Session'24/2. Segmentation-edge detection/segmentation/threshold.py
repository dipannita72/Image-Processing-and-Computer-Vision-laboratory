...
# Load the image
image = cv2.imread('coins2.jpg', cv2.IMREAD_GRAYSCALE)
print(image.min())
print(image.max())
...
binary_edge = global_thresholding(image)
hist = cv2.calcHist([binary_edge],[0],None,[256],[0,256]) 
plt.figure(4)
plt.plot(hist) 
plt.show()
